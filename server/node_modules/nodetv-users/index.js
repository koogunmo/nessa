...
var UserData = {
	...
	login: function(data){
		...
		userCollection.findOne({$or:[{'username':data.username},{'email':data.username}],'password':data.hashed},{'password':false,'sessions':false}, function(error,user){
			var response = {'error':null,'success':false,'session':null,'user':{}};
			if (error){
				logger.error(error);
				deferred.reject();
			}
			if (user){
				...
				var session = {'session':response.session,'timestamp':new Date()};
				userCollection.update({'_id':ObjectID(user._id)},{$push:{'sessions':session}},{'w':0});
				...
				if (user.trakt){
					trakt.auth(user.trakt).token().then(function(json){
						...
						try {
							...
							trakt.auth(user.trakt).users().settings().then(function(settings){
								var record = {
									'name': settings.user.name,
									'trakt.vip': settings.user.vip,
									'avatar': settings.user.images.avatar.full
								};
								userCollection.update({'_id':ObjectID(user._id)},{$set:record},{'w':0});
							}, function(error){
								logger.error(self.options, error);
							});
						} catch(e){
				...
				response.user = extend({},user);
				delete response.user.trakt;
				
				deferred.resolve(response);
			} else {
				response.error = 'Username and/or password is incorrect';
