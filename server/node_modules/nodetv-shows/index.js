...
var ShowData = {
	...
	scanShows: function(user, callback){
		...
		var insertShow = function(tvdb,dir){
			var tvdb = parseInt(tvdb,10);
			showCollection.update({'tvdb':tvdb},{$set:{'directory':dir}},{'upsert':true},function(error,affected){
				if (error) return logger.error(error);
				if (affected){
					self.add(user, tvdb).then(function(show){
						if (typeof(callback) == 'function') callback(error, show.tvdb);
					});
				}
			});
						...
						showCollection.findOne({$or:[{'name':dir},{'directory':dir}],'tvdb':{$exists:true}},function(error,show){
							...
							//	if (typeof(callback) == 'function') callback(error, show.tvdb);
								...
								trakt(user.trakt).search('shows', dir, function(error, results){
									...
									if (results){
										if (results.length == 1){
											insertShow(results[0].tvdb_id,dir);
										} else {
											...
											if (filtered.length == 1){
												insertShow(filtered[0].tvdb_id,dir);
											} else {
	...
	upcoming: function(user, callback){
		
		// TODO: Alternative for when trakt falls over
		
		trakt(user.trakt).calendar.shows(callback);
	},
	...
	getHashes: function(tvdb, callback){
		...
		showCollection.findOne({'tvdb':tvdb,'feed':{$exists:true,$ne:null}}, function(error, show){
			if (error) logger.error(error);
			if (show){
