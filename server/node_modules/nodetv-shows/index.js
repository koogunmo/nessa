...
var ShowData = {
	add: function(user,imdb){
		var self = this, deferred = Q.defer(), wait = Q.defer();
		var process = function(json){
			try {
				...
				showCollection.update({'imdb':record.imdb},{$set:record},{'upsert':true}, function(error,affected){
					if (error){
						logger.error(error);
					...
					if (affected){
						showCollection.findOne({'imdb':record.imdb}, function(error,show){
							...
							if (show) deferred.resolve(show);
							deferred.reject();
						});
						trakt.auth(user.trakt).show(record.imdb).watchlist(true);
					}
				});
			} catch(e){
		...
		trakt.show(imdb).summary().then(process).catch(function(error){
			if (error) logger.error(error);
			deferred.reject(error);
		});
		return deferred.promise;
	},
	checkin: function(user,imdb,json){
		...
		try {
			...
			episodeCollection.findOne({'downloading':{$exists:true},$or:[{'hash':hash},{'hashes.hash':hash}]}, function(error,episode){
				...
				if (episode){
					...
					if (!files.length) return deferred.reject();
					if (files.length == 1){
						file = files[0]
					} else {
						// Get the largest file
						files.forEach(function(item){
							if (parseInt(item.bytesCompleted) < size) return;
							file = item, size = parseInt(item.bytesCompleted);
						});
					}
											...
											userCollection.findOne({'_id':ObjectID(u._id),'trakt':{$exists:true}},{'trakt':1},function(error,user){
												...
												if (user){
													meta.episodes.forEach(function(episode){
														trakt.auth(user.trakt).show(show.imdb).season(meta.season).episode(episode).collected(true);
													});
												}
											});
										});
	...
	directory: function(imdb){
		// Create or rename the show directory
		var deferred = Q.defer(), self = this;
		
		var basedir = nconf.get('media:base')+nconf.get('media:shows:directory');
		showCollection.findOne({'imdb':imdb}, function(error,show){
			...
			if (show){
				if (!show.directory){
					var directory = helper.fileSanitize(show.name);
					mkdir(basedir+'/'+directory, 0775, function(error){
						if (error){
							logger.error(error);
							deferred.reject(error);
						} else {
							showCollection.update({'imdb':show.imdb},{$set:{'directory':directory}},{'w':0});
				...
				/*
				if (show.directory){
					// Disabled renaming until we can find a solution for Doctor Who, et al
					if (fs.existsSync(basedir+'/'+show.directory)) {
						deferred.resolve(show);
					} else {
						// Create directory
						var directory = helper.fileSanitize(show.name);
					...
					if (show.directory == directory){
						...
						// Rename directory
						helper.folderMove(basedir+'/'+show.directory,basedir+'/'+directory).then(function(){
							showCollection.update({'imdb':show.imdb},{$set:{'directory':directory}},{'w':0});
							show.directory = directory;
							deferred.resolve(show);
					...
					if (episodes){
						episodes.forEach(function(episode){
							try {
								...
								if (episode.users){
									for (var i in episode.users){
										if (episode.users[i].equals(user._id)) episode.watched = true;
									}
									delete episode.users;
								}
	...
	latest: function(user){
		var deferred = Q.defer(), self = this;
		var lastweek = new Date(), nextweek = new Date();
		...
		showCollection.find({'users._id': ObjectID(user._id)},{'progress':0,'seasons':0,'synopsis':0}).toArray(function(error, shows){
			...
			if (shows){
				shows.forEach(function(show){
					episodeCollection.find({$or:[{'tvdb':show.tvdb},{'show':ObjectID(show._id)}],'file':{$exists:true},'added':{$gte:lastweek},'airdate':{$gte:lastweek}}).toArray(function(error, episodes){
						count++
						if (error){
							logger.error(error);
							deferred.reject(error);
	...
	listings: function(user){
		...
		showCollection.find({'imdb':{$exists:true}}).toArray(function(error,shows){
			...
			if (shows){
				shows.forEach(function(show){
					self.getListings(show.imdb).then(function(show){
						return self.getProgress(user,show.imdb);
					}).catch(function(error){
						if (error) logger.error(error);
					})
				});
	...
	match: function(user, match){
		var deferred = Q.defer(), self = this;
		
		showCollection.update({'imdb':show.imdb},{$set:{'directory':match.directory}},{'upsert':true},function(error,affected){
			if (error){
				logger.error(error);
				deferred.reject(error);
			}
			if (affected){
				self.add(user,match.imdb).then(function(show){
					self.getFeed(show.imdb);
					return self.directory(show.imdb);
	...
	progress: function(user,imdb){
		logger.debug('nodetv-shows.progress has been deprecated');
		var deferred = Q.defer(), self = this;
		/*
		showCollection.findOne({'imdb':imdb,'users._id':ObjectID(user._id)}, function(error, show){
			if (error){
	...
	remove: function(user,imdb){
		...
		showCollection.findOne({'imdb':imdb,'users._id':ObjectID(user._id)},function(error,show){
			...
			if (show){
				...
				trakt.auth(user.trakt).show(show.imdb).collected(false);
				trakt.auth(user.trakt).show(show.imdb).watched(false);
	...
	scan: function(user){
		var self = this;
		this.scanShows(user, function(error,show){
			if (error) logger.error(error);
			if (show) self.scanEpisodes(show.imdb);
		});
	},
								...
								if (show.users){
									show.users.forEach(function(u){
										userCollection.findOne({'_id':ObjectID(u._id),'trakt':{$exists:true}},{'trakt':true}, function(error,user){
											...
											if (user){
												meta.episodes.forEach(function(episode){
													trakt.auth(user.trakt).show(show.imdb).season(meta.season).episode(episode).collected(true, record.added);
												});
											}
										});
									});
	...
	scanShows: function(user, callback){
		...
		var insertShow = function(imdb,source){
			showCollection.update({'imdb':imdb},{$set:{'directory':dir}},{'upsert':true}, function(error,affected){
				...
				if (affected){
					self.add(user,imdb).then(function(show){
						...
						self.getProgress(user,show.imdb);
						if (typeof(callback) == 'function') callback(null, show);
					}).catch(function(error){
						if (error) logger.error(error);
					});
				}
				...
				dirs.forEach(function(dir){
					fs.lstat(base + '/' + dir, function(error,stat){
						...
						if (stat && stat.isDirectory()){
							...
							showCollection.findOne({$or:[{'directory':dir},{'name':dir}],'imdb':{$exists:true}}, function(error,show){
								if (error) logger.error(error);
								if (show){
									if (!show.directory){
								...
								//	if (typeof(callback) == 'function') callback(null, show);
									...
									var query = dir.replace(/[^a-z0-9]/ig, ' ').toLowerCase();
									logger.debug('Searching for: %s', query);
									trakt.search('show', query).then(function(response){
										var unmatched = null, results = [];
										for (var i in response) results.push(response[i].show);
										
										if (results.length){
											if (results.length == 1){
												logger.debug('Exact match: %s (%d)', results[0].title, results[0].year);
												insertShow(results[0].ids.imdb,dir);
											} else {
												var filtered = results.filter(function(result){
													var include = true;
													var filtered = query.split(' ').forEach(function(word){
														var title = result.title.replace(/[^a-z0-9]/ig, ' ').toLowerCase().trim()
														if (title.indexOf(word) == -1) include = false;
													})
													return include;
												});
												if (filtered.length == 1){
													logger.debug('Filtered match: %s (%d)', filtered[0].title, filtered[0].year);
													insertShow(filtered[0].ids.imdb,dir);
												} else {
													unmatched = {'directory':dir,'matches':filtered};
												}
											}
										} else {
											unmatched = {'directory':dir};
										}
									//	if (unmatched) unmatchedCollection.update({'type':'show','directory':dir},{$set:unmatched},{'upsert':true,'w':0});
									}, function(error){
										if (error) logger.error(error);
									});
								}
	...
	search: function(user, query){
		return trakt.search('show', query);
	},
	...
	sync: function(user){
		...
		var syncShows = function(results){
			try {
				for (var i in results) updateShow(results[i].show);
			} catch(e){
				logger.error(e.message);
				deferred.reject(e.message);
			}
		...
		trakt.auth(user.trakt).sync().shows.watchlist().then(syncShows);
		...
		showCollection.findOne({'imdb':imdb,'users._id':ObjectID(user._id)}, function(error,show){
			...
			if (show){
				var updateStats = function(){
					showCollection.update({'imdb':show.imdb},{$set:{'resync':true}},{'w':0});
				};
				var search = {
					$or: [{'show':ObjectID(show._id)},{'tvdb':show.tvdb}]
				};
				...
				if (json.watched){
					episodeCollection.update(search,{$addToSet:{'users':ObjectID(user._id)}},{'multi':true,'w':0});
				} else {
					episodeCollection.update(search,{$pull:{'users':ObjectID(user._id)}},{'multi':true,'w':0});
				}
				deferred.resolve(show);
				
				if (typeof(json.season)!='undefined'){
					if (typeof(json.episode)!='undefined'){
						trakt.auth(user.trakt).show(show.imdb).season(json.season).episode(json.episode).watched(json.watched).then(updateStats);
					} else {
						trakt.auth(user.trakt).show(show.imdb).season(json.season).watched(json.watched).then(updateStats);
					}
				} else {
					trakt.auth(user.trakt).show(show.imdb).watched(json.watched).then(updateStats);
				}
			} else {
				deferred.reject();
			}
	...
	addUser: function(user,imdb){
		var deferred = Q.defer(), self = this
		var addUser = function(show, record){
			showCollection.update({'imdb':imdb},record,function(error,affected){
				if (error) {
					logger.error(error);
					deferred.reject(error);
				}
				if (affected) deferred.resolve(show);
				deferred.reject();
			});
		};
		showCollection.findOne({'imdb':imdb},function(error,show){
			...
			if (show){
				...
				if (show.users){
					...
					if (users.length) {
						...
						addUser(show, {$addToSet:{'users':record}});
					}
				} else {
					addUser(show, {$set:{'users':[record]}})
				}
			} else {
				deferred.reject();
			}
	...
	getArtwork: function(imdb){
		...
		showCollection.findOne({'imdb':imdb}, function(error, show){
			...
			if (show){
				if (!show.directory || show.directory == ''){
					deferred.reject();
					return;
				}
				var directory = nconf.get('media:base')+nconf.get('media:shows:directory')+'/'+show.directory;
				if (!fs.existsSync(directory)) mkdir.sync(directory,0775);
				...
				trakt.show(show.imdb).images().then(function(json){
					...
					if (json.images.banner.full){
						try {
							var banner = fs.createWriteStream(directory+'/banner.jpg', {'flags':'w','mode':0664});
	...
	getFilename: function(file,show){
		...
		var token = {'S':null,'E':null,'T':[],'X': settings.filext}, values = [];
		...
		showCollection.findOne({'imdb':imdb}, function(error,show){
			...
			if (show){
				trakt.auth(user.trakt).show(show.imdb).progress.watched().then(function(progress){
					...
					showCollection.update({'imdb':show.imdb,'users._id':ObjectID(user._id)},{
						$set:{'users.$.progress':record.progress,'users.$.seasons':record.seasons},
						$unset:{'resync':true}
					},{'w':0});
				...
				if (xml){
					try {
						parser.parseString(xml, function(error,json){
							...
							if (json){
								if (!json.rss.channel[0].item) return deferred.reject();
								var hashes = [];
