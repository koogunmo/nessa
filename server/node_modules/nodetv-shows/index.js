...
var ShowData = {
	...
	directory: function(imdb){
		...
		showCollection.findOne({'imdb':imdb}, function(error,show){
			...
			if (show){
				var directory = helper.fileSanitize(show.title);
				if (show.directory){
					if (show.directory == directory){
						...
						// Rename directory
						helper.folderMove(basedir+'/'+show.directory,basedir+'/'+directory);
						show.directory = directory;
						deferred.resolve(show);
					}
				} else {
	...
	setProgress: function(user,imdb){
		// TODO: update the local progress details (on the show document) to match what *should* be on Trakt 
		// (We can't hammer the sync endpoints every time they perform an action)
		var deferred = Q.defer(), self = this;
		showCollection.findOne({'imdb':imdb,'users._id':ObjectID(user._id)},function(error,show){
			if (error){
				logger.error(error);
			...
			if (show){
				...
				if (!users.length){
					// Okay, NOW we can hit the endpoint
					self.getProgress(user,imdb).then(function(success){
						deferred.resolve(success);
					}, function(error){
						deferred.reject(error);
					});
				} else if (users.length == 1){
					// Recalculate watched status, reticulate splines
					if (!users[0].progress || !users[0].seasons){
						self.getProgress(user,imdb).then(function(success){
							deferred.resolve(success);
						}, function(error){
							deferred.reject(error);
						...
						var update = {
							'progress':{'percentage':0,'aired':0,'completed':0,'left':0},
							'seasons':[{'season':0},{'season':1}]
						};
