...
var TraktAPI = module.exports = function(config){
	...
	this.api_url = (this.settings.testing) ? this.urls.staging : this.urls.production;
...
TraktAPI.prototype.post = function(url, data){
	var deferred = Q.defer(), options = {}, self = this;
	if (!data.app_version) data.app_version = pkg.version;
	options.body = data;
...
TraktAPI.prototype.token = function(){
	var deferred = Q.defer(), self = this;
	this.request.post(this.api_url+'/auth/login', {'body':{'login':this.user.username,'password':this.user.password}}, function(error,res,json){
		if (error){
			logger.error(error);
			deferred.reject();
		}
		if (res && res.statusCode >= 400) {
			deferred.reject({'status':res.statusCode,'error':res.headers.status,'url':self.api_url+'/auth/login'});
		}
		if (json){
...
TraktAPI.prototype.genres = function(){
	var self = this;
	return {
		'movies': function(){
			return self.get(self.api_url+'/genres/movies');
		},
		'shows': function(){
			return self.get(self.api_url+'/genres/shows');
		}
	};
};
...
TraktAPI.prototype.movie = function(id){
	...
	return {
		'collected': function(status,time){
			...
			return self.post(self.api_url+endpoint, payload);
		},
		'images': function(){
			return self.get(self.api_url+'/movies/'+id+'?extended=images');
		},
		'ratings': function(){
			return self.get(self.api_url+'/movies/'+id+'/ratings');
		},
		'search': function(){
			return self.get(self.api_url+'/search?type=movie&query='+query);
		},
		'summary': function(){
			return self.get(self.api_url+'/movies/'+id+'?extended=full');
		},
		...
		'watching': function(){
			return self.get(self.api_url+'/movies/'+id+'/watching');
		},
...
TraktAPI.prototype.search = function(type, query){
	return this.get(this.api_url+'/search?type='+type+'&query='+query);
};
	...
	return {
		...
		'episode': function(season,episode){
			return {
				...
				'summary': function(){
					return self.get(self.api_url+'/shows/'+id+'/seasons/'+season+'/episodes/'+episode+'?extended=full');
				},
				'watched': function(status){
					status = (typeof(status) == 'undefined') ? true : status;
					var payload = {
		...
		'images': function(){
			return self.get(self.api_url+'/shows/'+id+'?extended=images');
		},
		'progress': {
			'collected': function(){
				return self.get(self.api_url+'/shows/'+id+'/progress/collection');
			},
			'watched': function(){
				return self.get(self.api_url+'/shows/'+id+'/progress/watched');
			}
		},
		'search': function(){
			return self.get(self.api_url+'/search?type=show&query='+query);
		},
			...
			return {
				...
				'episode': function(episode){
					return {
						...
						'images': function(){
							return self.get(self.api_url+'/shows/'+id+'/seasons/'+season+'/episodes/'+episode+'?extended=images');
						},
						'summary': function(){
							return self.get(self.api_url+'/shows/'+id+'/seasons/'+season+'/episodes/'+episode+'?extended=full');
						},
				...
				'images': function(){
					return self.get(self.api_url+'/shows/'+id+'/seasons/'+season+'?extended=images');
				},
				'summary': function(){
					return self.get(self.api_url+'/shows/'+id+'/seasons/'+season+'?extended=full');
				},
				'watched': function(status){
					status = (typeof(status) == 'undefined') ? true : status;
					var payload = {
		...
		'seasons': function(){
			return self.get(self.api_url+'/shows/'+id+'/seasons?extended=full');
		},
		'summary': function(){
			return self.get(self.api_url+'/shows/'+id+'?extended=full');
		},
		'watched': function(status){
			status = (typeof(status) == 'undefined') ? true : status;
			var payload = {
...
TraktAPI.prototype.sync = function(){
	...
	return {
		'movies': {
			'collection': function(){
				return self.get(self.api_url+'/sync/collection/movies?extended=full');
			},
			'watchlist': function(){
				return self.get(self.api_url+'/sync/watchlist/movies?extended=full');
			}
		},
		'shows': {
			'collection': function(){
				return self.get(self.api_url+'/sync/collection/shows?extended=full');
			},
			'watchlist': function(){
				return self.get(self.api_url+'/sync/watchlist/shows?extended=full');
			}
		}
	};
};

TraktAPI.prototype.users = function(){
	var self = this;
	return {
		'settings': function(){
			return self.get(self.api_url+'/users/settings');
		}
	};	
};