...
TraktAPI.prototype.deauth = function(){
	try {
		this.user = false;
...
TraktAPI.prototype.login = function(){
	...
	if (this.user == false) {
		...
		if (this.user.token) {
			...
			self.request.post(api_url+'/auth/login', {'body':{'login':this.user.username,'password':this.user.password}}, function(error,res,json){
				if (error) return deferred.reject(error);
				if (json){
					if (json.error){
	...
	return {
		...
		'season': function(season){
			return {
				...
				'images': function(){
					return self.get(api_url+'/shows/'+id+'/season/'+season+'?extended=images');
				},
				'summary': function(){
...
TraktAPI.prototype.sync = function(){
	...
	switch (type.toLowerCase()){
		...
		case 'shows':
			...
			break;
		case 'watchlist':
			return this.get(api_url+'/sync/watchlist/movies');
			break;
	};
	*/
	return {
		'movies': {
			'collection': function(){
				return self.get(api_url+'/sync/collection/movies');
			},
			'watchlist': function(){
				return self.get(api_url+'/sync/watchlist/movies');
			}
		},
		'shows': {
			'collection': function(){
				return self.get(api_url+'/sync/collection/shows');
			},
			'watchlist': function(){
				return self.get(api_url+'/sync/watchlist/shows');
			}
		}
	};
};