...
var MovieData = {
	...
	link: function(imdb){
		...
		movieCollection.findOne({'imdb':imdb}, function(error, movie){
			...
			if (movie){
				logger.trace('Symlinking:', movie.title);
				var basedir = nconf.get('media:base')+nconf.get('media:movies:directory');
				...
				movie.genres.forEach(function(genre){
					...
					fs.symlink(basedir+'/A-Z/'+movie.file, folder+'/'+filename, 'file', function(error){
						...
						logger.trace('Linked: %s/%s', genre, movie.filename);
					});
				});
				deferred.resolve(movie)
			} else {
	...
	match: function(user, matched){
		...
		self.add(user, matched.imdb).then(function(movie){
			...
			logger.trace('Removing unmatched item');
	...
	rename: function(user, imdb, file){
		...
		movieCollection.findOne({'imdb':imdb}, function(error, movie){
			...
			if (movie){
				logger.trace('Renaming:', movie.title);
	...
	scan: function(user){
		var deferred = Q.defer(), self = this;
		logger.trace('Scanning movie library...');
			...
			movieCollection.findOne({'file':filename}, function(error,movie){
				...
				if (!movie){
					...
					trakt.search('movie', title).then(function(response){
						...
						if (results.length){
							if (results.length == 1){
								logger.trace('Exact:', filtered[0].title, filtered[0].year);
								...
								if (filtered.length == 1){
									logger.trace('Filtered:', filtered[0].title, filtered[0].year);
