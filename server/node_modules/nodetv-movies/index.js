...
var MovieData = {
	...
	match: function(user, matched){
		...
		self.add(user, matched.imdb).then(function(movie){
			...
			logger.debug('Added:', movie.title);
			
			self.rename(user, matched.imdb, basedir+matched.file).then(function(){
				logger.debug('remove queue record', {'type':'movie','file':matched.file});
				unmatchedCollection.remove({'type':'movie','file':matched.file},function(error,affected){
					if (error) logger.error(error);
					logger.debug(affected);
				});
				self.link(matched.imdb);
			}, function(error){
	...
	scan: function(user, callback){
		...
		var insertMovie = function(imdb,file){
			self.add(user,imdb).then(function(movie){
				if (movie){
					self.rename(user, movie.imdb, file.path).then(function(){
			...
			movieCollection.findOne({$or:[{'file':filename},{'title':title}]}, function(error,movie){
				...
				//	logger.info('Found:', movie.title);
					...
					trakt.search('movie', title).then(function(response){
						...
						if (results.length){
							if (results.length == 1){
								logger.debug('Exact:', filename, results[0].movie.title);
