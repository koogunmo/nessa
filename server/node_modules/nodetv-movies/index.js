...
var MovieData = {
	...
	match: function(user, matched){
		...
		self.add(user, matched.imdb).then(function(movie){
			...
			logger.debug('Removing unmatched item');
			...
			if (movie){
				...
				if (target == file){
					...
					helper.fileMove(file, target).then(function(to){
						deferred.resolve(extend({},movie,record));
			...
			movieCollection.findOne({'file':filename}, function(error,movie){
				...
				if (!movie){
					...
					trakt.search('movie', title).then(function(response){
						...
						if (results.length){
							if (results.length == 1){
								logger.debug('Exact:', filtered[0].title, filtered[0].year);
								...
								if (filtered.length == 1){
									logger.debug('Filtered:', filtered[0].title, filtered[0].year);
	...
	unlink: function(imdb){
		...
		movieCollection.findOne({'imdb':imdb},function(error, movie){
			...
			if (movie){
				logger.debug('Removing symlinks:', movie.title);
				var basedir = nconf.get('media:base')+nconf.get('media:movies:directory');
				if (movie.file && movie.genres){
					...
					movie.genres.forEach(function(genre,i){
						try {
							genre = genre.replace('-',' ').capitalize();
							fs.unlinkSync(basedir+'/Genres/'+genre+'/'+filename);
				...
				if (stat.isDirectory()){
					...
					if (files){
						files.forEach(function(file){
							logger.debug(directory+'/'+dir+'/'+file);
