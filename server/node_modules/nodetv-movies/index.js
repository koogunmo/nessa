...
var MovieData = {
	add: function(user, imdb, callback){
		...
		trakt(user.trakt).movie.summary(imdb).then(addMovie,function(error){
			logger.error(error);
		});
	...
	remove: function(user, tmdb, callback){
		...
		movieCollection.findOne({'tmdb':tmdb,'users._id':ObjectID(user._id)}, function(error,movie){
			if (error) logger.error(error);
			if (movie){
				if (physical){
					...
					movieCollection.update({'tmdb':movie.tmdb},update,{'w':0});
					deferred.resolve();
				}
			} else {
				deferred.reject(error);
	...
	scan: function(user, callback){
		...
		var insertMovie = function(imdb,file){
			self.add(user,imdb).then(function(movie){
				if (movie){
					self.rename(user, movie.tmdb, file.path, self.getQuality(path.basename(file.path)));
		...
		helper.listDirectory(base, function(error, file){
			...
			movieCollection.findOne({$or:[{'file':filename},{'title':title}]}, function(error,movie){
				...
				//	logger.info('Found:', movie.title);
					...
					trakt(user.trakt).search('movies', title).then(function(results){
						var unmatched = null;
						if (results){
							if (results.length == 1){
								insertMovie(results[0].imdb_id, file);
							} else {
